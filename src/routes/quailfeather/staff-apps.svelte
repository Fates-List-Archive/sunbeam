...
<QuailTree perms={perms.perm}>
	<Section id="sapps" title="Staff Apps" icon="fluent:thinking-24-regular">
		<div class="hr-sep">
			{#if perms.perm < 2}
				<p>The below list of staff applications only shows applications that you have made that are pending review</p>
			{:else}
	...
	<Section id="sapps-apply" title="Apply" icon="fa-solid:certificate">
		...
		{#if questions.can_apply}
			...
			<Button href={"javascript:void(0)"} class="mx-2 button" variant="outlined" on:click={() => {
				let inputs = [];
