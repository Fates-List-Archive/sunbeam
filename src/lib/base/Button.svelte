...
{#if href && useLink}
    <a class={"_ia"} href={href} target={target}>
        <button style={style} aria-label={ariaLabel} aria-disabled={ariaDisabled} class={className} on:click={onclick} id={id}>
...
{:else}
    <button style={style} aria-label={ariaLabel} aria-disabled={ariaDisabled} class={className} on:click={onclick}>
        <span class="button-text">
            <slot></slot>
        </span>
    </button>
